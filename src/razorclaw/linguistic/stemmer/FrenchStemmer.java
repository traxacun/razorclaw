package razorclaw.linguistic.stemmer;
...
/**
 * A stemmer for French words. The algorithm is based on the work of Dr Martin
 * Porter on his snowball project<br>
...
public class FrenchStemmer extends Stemmer {
	 ...
	 */
	private final StringBuffer tb = new StringBuffer();
	...
	/**
	 * Region RV "If the word begins with two vowels, RV is the region after the
	...
	/**
	 * Region R2 "R2 is the region after the first non-vowel in R1 following a
	 * vowel or is the null region at the end of the word if there is no such
	 * non-vowel"
	 */
	private String R2;
	 ...
	 * refer to http://snowball.sourceforge.net/french/stemmer.html for an
	 * explanation
	 */
	private void step1() {
		String[] suffix = { "ances", "iqUes", "ismes", "ables", "istes",
				"ance", "iqUe", "isme", "able", "iste" };
		deleteFrom(R2, suffix);

		replaceFrom(R2, new String[] { "logies", "logie" }, "log");
		replaceFrom(R2, new String[] { "usions", "utions", "usion", "ution" },
				"u");
		replaceFrom(R2, new String[] { "ences", "ence" }, "ent");

		String[] search = { "atrices", "ateurs", "ations", "atrice", "ateur",
				"ation" };
		deleteButSuffixFromElseReplace(R2, search, "ic", true, R0, "iqU");

		deleteButSuffixFromElseReplace(R2, new String[] { "ements", "ement" },
				"eus", false, R0, "eux");
		deleteButSuffixFrom(R2, new String[] { "ements", "ement" }, "ativ",
				false);
		deleteButSuffixFrom(R2, new String[] { "ements", "ement" }, "iv", false);
		deleteButSuffixFrom(R2, new String[] { "ements", "ement" }, "abl",
				false);
		deleteButSuffixFrom(R2, new String[] { "ements", "ement" }, "iqU",
				false);
		...
		deleteButSuffixFromElseReplace(R2, new String[] { "it�s", "it�" },
				"abil", false, R0, "abl");
		deleteButSuffixFromElseReplace(R2, new String[] { "it�s", "it�" },
				"ic", false, R0, "iqU");
		...
		deleteButSuffixFromElseReplace(R2, new String[] { "euses", "euse" },
				"", true, R1, "eux");
		...
		if (temp == true)
			suite = true;
		temp = deleteFromIfTestVowelBeforeIn(RV,
				new String[] { "ments", "ment" }, true, RV);
		if (temp == true)
			suite = true;
	 ...
	 * Will be performed if nothing changed from the first step or changed were
	 * done in the amment, emment, ments or ment suffixes<br>
	 * refer to http://snowball.sourceforge.net/french/stemmer.html for an
	 * explanation
	 * 
	...
	private boolean step2a() {
		String[] search = { "�mes", "�tes", "iraIent", "irait", "irais",
				"irai", "iras", "ira", "irent", "iriez", "irez", "irions",
				"irons", "iront", "issaIent", "issais", "issantes", "issante",
				"issants", "issant", "issait", "issais", "issions", "issons",
				"issiez", "issez", "issent", "isses", "isse", "ir", "is", "�t",
				"it", "ies", "ie", "i" };
	...
	private void step2b() {
		String[] suffix = { "eraIent", "erais", "erait", "erai", "eras",
				"erions", "eriez", "erons", "eront", "erez", "�rent", "era",
				"�es", "iez", "�e", "�s", "er", "ez", "�" };
		deleteFrom(RV, suffix);

		String[] search = { "assions", "assiez", "assent", "asses", "asse",
				"aIent", "antes", "aIent", "Aient", "ante", "�mes", "�tes",
				"ants", "ant", "ait", "a�t", "ais", "Ait", "A�t", "Ais", "�t",
				"as", "ai", "Ai", "a" };
	...
	private void step4() {
		...
		boolean found = deleteFromIfPrecededIn(R2, new String[] { "ion" }, RV,
				"s");
		if (!found)
	...
	/**
	 * Delete a suffix searched in zone "source" if zone "from" contains prefix
	 * + search string
	 * 
	 * @param source
	 *            java.lang.String - the primary source zone for search
	 * @param search
	 *            java.lang.String[] - the strings to search for suppression
	 * @param from
	 *            java.lang.String - the secondary source zone for search
	 * @param prefix
	 *            java.lang.String - the prefix to add to the search string to
	 *            test
	 * @return boolean - true if modified
	 */
	private boolean deleteFromIfPrecededIn(String source, String[] search,
	...
	/**
	 * Delete a suffix searched in zone "source" if the preceding letter is (or
	 * isn't) a vowel
	 * 
	 * @param source
	 *            java.lang.String - the primary source zone for search
	 ...
	 *            java.lang.String[] - the strings to search for suppression
	 * @param vowel
	 *            boolean - true if we need a vowel before the search string
	 * @param from
	 *            java.lang.String - the secondary source zone for search (where
	 *            vowel could be)
	 * @return boolean - true if modified
	 */
	private boolean deleteFromIfTestVowelBeforeIn(String source,
			String[] search, boolean vowel, String from) {
		boolean found = false;
		if (source != null && from != null) {
			for (int i = 0; i < search.length; i++) {
				if (source.endsWith(search[i])) {
					if ((search[i].length() + 1) <= from.length()) {
						boolean test = isVowel(sb.charAt(sb.length()
								- (search[i].length() + 1)));
						if (test == vowel) {
							sb.delete(sb.length() - search[i].length(),
									sb.length());
							modified = true;
							found = true;
							setStrings();
	 ...
	 *            test
	 * @param without
	 *            boolean - true if it will be deleted even without prefix found
	 */
	private void deleteButSuffixFrom(String source, String[] search,
			String prefix, boolean without) {
		if (source != null) {
			for (int i = 0; i < search.length; i++) {
				if (source.endsWith(prefix + search[i])) {
					sb.delete(
							sb.length()
									- (prefix.length() + search[i].length()),
							sb.length());
					modified = true;
					setStrings();
					break;
	 ...
	 * or replace it with the replace string if preceded by the prefix in the
	 * zone "from"<br>
	 ...
	 *            boolean - true if it will be deleted even without prefix found
	 */
	private void deleteButSuffixFromElseReplace(String source, String[] search,
			String prefix, boolean without, String from, String replace) {
		if (source != null) {
			for (int i = 0; i < search.length; i++) {
				if (source.endsWith(prefix + search[i])) {
					sb.delete(
	 ...
	 * 
	 * @param source
	 *            java.lang.String - the source zone for search
	 * @param search
	 *            java.lang.String[] - the strings to search for replacement
	 * @param replace
	...
	private boolean replaceFrom(String source, String[] search, String replace) {
		...
		if (source != null) {
			for (int i = 0; i < search.length; i++) {
				if (source.endsWith(search[i])) {
					sb.replace(sb.length() - search[i].length(), sb.length(),
							replace);
					modified = true;
					found = true;
					setStrings();
	...
	/**
	 * Retrieve the "R zone" (1 or 2 depending on the buffer) and return the
	 * corresponding string<br>
	 * "R is the region after the first non-vowel following a vowel or is the
	 * null region at the end of the word if there is no such non-vowel"<br>
	 * 
	 * @param buffer
	 *            java.lang.StringBuffer - the in buffer
	 ...
	 * of the word, or the end of the word if these positions cannot be found."<br>
	 * 
	 * @param buffer
	 *            java.lang.StringBuffer - the in buffer
	 * @return java.lang.String - the resulting string
			...
			} else // other cases
			{
				if (ch == 'u') {
					if (buffer.charAt(c - 1) == 'q')
						buffer.setCharAt(c, 'U');
					else if (isVowel(buffer.charAt(c - 1))
							&& isVowel(buffer.charAt(c + 1)))
						buffer.setCharAt(c, 'U');
				}
				if (ch == 'i') {
					if (isVowel(buffer.charAt(c - 1))
							&& isVowel(buffer.charAt(c + 1)))
	 ...
	 * 
	 * @return boolean - true if, and only if, the given term consists in
	 *         letters.
	 */
